...
//Assignment 2 refactor.txt - eslamif 5 card functions:
int adventurer_refactor(int currentPlayer, struct gameState *state) {
...
int remodel_refactor(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2) {
      int i, j;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		if((adventurer_refactor(currentPlayer, state)) == 0)	
				return 0;
		else
				exit(1);

    case council_room:
		if((council_room_refactor(currentPlayer, state, handPos)) == 0)	
				return 0;
		else
				exit(1);
    ...
    case remodel:
		if((remodel_refactor(currentPlayer, state, handPos, choice1, choice2)) == 0)	
				return 0;
		else
				exit(1);

    case smithy:
		if((smithy_refactor(currentPlayer, state, handPos)) == 0)	
				return 0;
		else
				exit(1);
